...
class D0Policy:
    ...
    def warmup(self, runs: int = 1) -> dict[str, Any]:
        """Run the deployed action path with synthetic observations before serving."""
        runs = int(runs)
        if runs < 0:
            raise ValueError("warmup runs must be non-negative")
        if runs == 0:
            return {"runs": 0, "timings_ms": []}

        spec = next(iter(self.task_specs.values()))
        image = np.zeros((480, 640, 3), dtype=np.uint8)
        horizon = int(self.config["common"]["num_video_frames"]) * int(
            self.config["common"]["video_action_freq_ratio"]
        )
        observation = {
            "state": np.zeros(14, dtype=np.float32),
            "front_camera": image,
            "left_wrist_camera": image,
        ...
        if flow_mode == "history":
            observation["history_actions"] = np.zeros((horizon, 14), dtype=np.float32)
        ...
        try:
            for index in range(runs):
                ...
                logger.info("D0 warmup %d/%d completed in %.2f ms", index + 1, runs, elapsed_ms)
        finally:
            torch.random.set_rng_state(cpu_rng_state)
            if cuda_rng_state is not None:
                torch.cuda.set_rng_state(cuda_rng_state, self.device)

        return {"runs": runs, "task_id": spec.task_id, "timings_ms": timings_ms}
