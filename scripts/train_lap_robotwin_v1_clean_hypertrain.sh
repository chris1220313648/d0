...
# HyperTrain launcher for RobotWin LAP v1 clean finetuning.
# This wrapper reuses the shared RobotWin HyperTrain launcher and only changes
# the default task/config/run naming.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export TASK="${TASK:-robotwin_lap_v1_clean}"
export CONFIG_FILE="${CONFIG_FILE:-configs/robotwin_lap_v1_clean.yaml}"
