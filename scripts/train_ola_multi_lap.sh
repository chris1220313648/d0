...
# GPUs must already be allocated by the user before this script is launched.
CONFIG_FILE="${CONFIG_FILE:-configs/ola_multi_lap.yaml}"
DEEPSPEED_CONFIG="${DEEPSPEED_CONFIG:-configs/zero2.json}"
RUN_NAME="${RUN_NAME:-ola_multi_lap_d0_v_after_pretrain}"
REPORT_TO="${REPORT_TO:-wandb}"
NPROC_PER_NODE="${NPROC_PER_NODE:-8}"
MASTER_ADDR="${MASTER_ADDR:-127.0.0.1}"
MASTER_PORT="${MASTER_PORT:-29500}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/motus-ola-multi-lap}"
export OUTPUT_DIR
