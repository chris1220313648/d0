...
from models.motus import Motus, UndModule
...
def test_euler_inference_extracts_vlm_once_and_resets_tokens_each_step():
    model = _build_fake_motus()

    frames, actions = model.inference_step(
    ...
    assert model.video_model.decode_calls == 0
...
def test_euler_inference_decodes_video_by_default():
    model = _build_fake_motus()

    frames, _ = model.inference_step(
        first_frame=torch.zeros((1, 3, 4, 4)),
        state=torch.zeros((1, 14)),
    ...
    assert model.video_model.decode_calls == 1
...
class _FakeUndExpert(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.vlm_adapter = torch.nn.Identity()


def test_extract_und_features_uses_last_hidden_state_without_requesting_all_layers():
    expected = torch.randn((1, 3, 4), requires_grad=True)
    language_model = _FakeLanguageModel(expected)
    vlm_model = SimpleNamespace(model=SimpleNamespace(language_model=language_model))
    module = UndModule(
        vlm_model=vlm_model,
        und_expert=_FakeUndExpert(),
        config=SimpleNamespace(vlm_frozen=False),
        dtype=torch.float32,
        device=torch.device("cpu"),
    )
    module._process_vlm_inputs_to_tokens = lambda inputs, batch: (
        torch.zeros((1, 3, 4)),
        torch.ones((1, 3)),
        None,
        None,
    ...
    features, loss = module.extract_und_features(
        {"input_ids": torch.zeros((1, 3), dtype=torch.long)}
    )

    assert loss is None
    assert features is expected
    assert language_model.kwargs["output_hidden_states"] is False
