...
from deploy.serve_d0 import load_deployment, parse_args
...
def test_policy_warmup_uses_deployment_contract_and_preserves_cpu_rng():
    policy = object.__new__(D0Policy)
    policy.config = {
        "common": {"num_video_frames": 8, "video_action_freq_ratio": 6},
        "model": {"flow_source": {"mode": "history"}},
    ...
    policy.task_specs = {
        "test": TaskSpec("test", "do it", "/tmp/not-read-by-fake-infer.pt")
    }
    policy.device = torch.device("cpu")
    observations = []
    ...
    def fake_infer(observation):
        ...
        torch.rand(1)
        return {"actions": np.zeros((48, 14), dtype=np.float32)}

    policy.infer = fake_infer
    torch.manual_seed(1234)
    rng_before = torch.random.get_rng_state().clone()

    result = policy.warmup(2)
    ...
    assert observation["history_actions"].shape == (48, 14)
    assert observation["front_camera"].shape == (480, 640, 3)
    assert observation["task_id"] == "test"
    assert observation["prompt"] == "do it"


def test_policy_warmup_can_be_disabled_and_rejects_negative_runs():
    policy = object.__new__(D0Policy)
    assert policy.warmup(0) == {"runs": 0, "timings_ms": []}
    with pytest.raises(ValueError, match="non-negative"):
        policy.warmup(-1)


def test_serve_defaults_to_one_warmup_run(monkeypatch):
    monkeypatch.setattr(sys, "argv", ["serve_d0.py", "--checkpoint", "/tmp/checkpoint"])
    assert parse_args().warmup_runs == 1
