...
# -*- coding: utf-8 -*-
"""
Convert Bridge RLDS TFRecord dataset into Motus-style directory layout.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Convert Bridge RLDS to Motus-style dataset.")
    ...
    parser.add_argument("--splits", nargs="+", default=None, help="Splits to convert, e.g. train test.")
    parser.add_argument(
...
def build_config(args: argparse.Namespace) -> ConvertConfig:
    ...
    return ConvertConfig(
        ...
        task_grouping=str(cfg_raw.get("task_grouping", "normalized_instruction")),
...
def _try_import_tf(disable_tf_gpu: bool = True) -> Tuple[Any, Any]:
    ...
    # Important:
    # TFDS is only used for reading RLDS records; keep TensorFlow off GPU by default
    # so PyTorch can use GPU memory for WAN T5 encoding.
    if disable_tf_gpu:
        try:
        ...
        # If TF GPU is enabled, at least avoid grabbing all memory upfront.
        try:
...
def ensure_episode_arrays(episode_np: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray, str]:
    ...
    # Case A: steps is a dict of stacked arrays/tensors.
    if isinstance(steps, dict):
        ...
        states = obs.get("state", None)
        instr = obs.get("natural_language_instruction", None)

        if images is None or states is None:
            raise KeyError("Episode observation must contain image and state.")

        images_np = np.asarray(images)
        states_np = np.asarray(states)
        instruction = first_non_empty_instruction(instr)
    else:
        # Case B: steps is an iterable dataset (common in RLDS/TFDS episode datasets).
        ...
        for step in steps:
            ...
            if img is None or st is None:
                raise KeyError("Step observation must contain image and state.")

            image_list.append(np.asarray(img))
            state_list.append(np.asarray(st))
            instr_list.append(obs.get("natural_language_instruction", None))

        if not image_list or not state_list:
            raise ValueError("Episode has no valid steps with image/state.")
    ...
    if states_np.ndim != 2:
        raise ValueError(f"Invalid state shape: expected [T,D], got {states_np.shape}")
    if images_np.shape[0] != states_np.shape[0]:
        raise ValueError(
            f"Frame/state length mismatch: images T={images_np.shape[0]}, states T={states_np.shape[0]}"
        )
    if images_np.shape[0] < 2:
        raise ValueError(f"Episode too short: T={images_np.shape[0]}")

    return images_np, states_np, instruction
...
def save_text_embedding_pt(processor: Any, instruction: str, out_path: str) -> None:
    ...
    # Reuse processor API by writing a one-line temp text file and calling process_meta_file.
...
def _episode_outputs_exist(video_path: str, qpos_path: str, t5_path: str, instruction_path: str) -> bool:
    return (
        os.path.exists(video_path)
        and os.path.exists(qpos_path)
        and os.path.exists(t5_path)
        and os.path.exists(instruction_path)
    )


def create_t5_processor(wan_repo_path: str, t5_max_length: int, device: str) -> Any:
    # Ensure Motus project root is importable no matter where the script is launched.
    # bridge_rlds_to_motus.py is under <repo>/data/bridge, so parents[2] is repo root.
...
def convert_split(
    ...
    for episode_idx, episode in enumerate(tfds.as_numpy(ds_split)):
        ...
        try:
            images_np, states_np, instruction = ensure_episode_arrays(episode)
            ...
            exists = _episode_outputs_exist(video_path, qpos_path, t5_path, instruction_path)
            ...
            torch.save(torch.from_numpy(states_np).float(), qpos_path)
...
def main() -> None:
    ...
    logger.info("Bridge RLDS conversion config:")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for WAN T5 embedding generation, but no CUDA device is available.")
