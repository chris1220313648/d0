# DROID Bridge-Style Dataset Loader for Motus
# Uses qpos as initial_state (absolute) and epos as action supervision (delta-like).
...
class DroidDataset(data.Dataset):
    """
    DROID dataset with Bridge-style semantics for Motus training.
    ...
    def __init__(
        self,
        dataset_dir: str = "/data/user/wsong890/user68/cjy/Motus/data/droid/droid_dataset",
        ...
        setup_text: str = "single-arm Franka Panda robot with parallel gripper",
        ...
        **kwargs,
        ...
        if self.normalize_actions:
            ...
            dataset_key = self.stats_key or "droid"
        ...
        logger.info("Droid dataset initialized:")
        ...
        if self.normalize_actions:
            logger.info(
                f"  Stats key/path/signals: {self.stats_key or 'droid'} / {stats_path} / "
    ...
    def _load_episodes(self) -> None:
        ...
        if self.task_mode == "single":
            ...
            logger.info(f"Multi-task DroidBridge dataset with {num_tasks} tasks.")
