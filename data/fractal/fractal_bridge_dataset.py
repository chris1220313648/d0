# Fractal Bridge-Style Dataset Loader for Motus
# Uses qpos as initial_state (absolute) and epos as action supervision (delta).
...
class FractalDataset(data.Dataset):
    """
    Fractal dataset with Bridge-style semantics for Motus training.
    ...
    def __init__(
        self,
        dataset_dir: str = "/data/user/wsong890/user68/cjy/Motus/data/fractal/fractal_dataset",
        ...
        setup_text: str = "Google RT-1 mobile manipulator robot with gripper",
        randomized_limit_per_task: Optional[int] = None,  # kept for config compatibility; unused here
        **kwargs,
    ):
        ...
        if self.normalize_actions:
            ...
            dataset_key = self.stats_key or "fractal"
        ...
        logger.info("Fractal dataset initialized:")
        ...
        if self.normalize_actions:
            logger.info(
                f"  Stats key/path/signals: {self.stats_key or 'fractal'} / {stats_path} / "
    ...
    def _load_episodes(self) -> None:
        ...
        if self.task_mode == "single":
            ...
            logger.info(f"Multi-task Fractal dataset with {num_tasks} tasks.")
    ...
    def _load_initial_state_and_actions(
        ...
        # initial_state uses absolute qpos.
...
FractalBridgeDataset = FractalDataset
