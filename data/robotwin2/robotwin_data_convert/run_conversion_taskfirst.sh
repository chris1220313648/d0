...
echo "Starting RobotWin task-first dataset conversion at $(date)"
...
LOG_FILE="${LOG_DIR}/conversion_taskfirst_${TIMESTAMP}.log"
...
# Run the conversion
echo "Executing task-first conversion script..."
cd "$SCRIPT_DIR"

$PYTHON_CMD robotwin_converter_taskfirst.py \
    --config "$CONFIG_FILE" \
    $VERBOSE_FLAG \
    2>&1 | tee "$LOG_FILE"
