...
class LeRobotOLAV3Dataset(Dataset):
    ...
    def __init__(
        ...
        included_episode_indices: Optional[Sequence[int]] = None,
        ...
        **_: Any,
        ...
        self.included_episode_indices = (
            {int(index) for index in included_episode_indices}
            if included_episode_indices is not None
            else None
        )
        ...
        if self.included_episode_indices is not None:
            if max_episodes is not None and int(max_episodes) > 0:
                raise ValueError("included_episode_indices and max_episodes are mutually exclusive")
            available = {episode.episode_index for episode in episodes}
            missing = sorted(self.included_episode_indices - available)
            if missing:
                raise ValueError(
                    f"Requested OLA episode indices are unavailable after manifest filters: {missing}"
                )
            episodes = [
                episode
                for episode in episodes
                if episode.episode_index in self.included_episode_indices
            ]
        if max_episodes is not None and int(max_episodes) > 0:
        ...
        logger.info(
            ...
            "excluded=%s included_count=%s task_filter=%s",
            self.root,
            len(self.episodes),
            len(self),
            self.state_key,
            ...
            len(self.included_episode_indices) if self.included_episode_indices is not None else None,
    ...
    def _history_indices(self, condition: int) -> List[int]:
        """Return a causal, fixed-length history ending at the condition frame.

        At the beginning of an episode the first recorded action is repeated;
        indices never cross the current episode boundary.
        """
        return [
            max(
                0,
                int(condition)
                - (self.history_action_length - 1 - index) * self.global_downsample_rate,
            )
            for index in range(self.history_action_length)
        ]
        ...
        if jsonl_path.exists():
            for line in jsonl_path.read_text(encoding="utf-8").splitlines():
                if not line.strip():
                    continue
                row = json.loads(line)
                actions[int(row["frame_index"])] = str(row["text"])
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if history_action_sequence is not None:
            sample["history_action_sequence"] = history_action_sequence.float()
        return sample
